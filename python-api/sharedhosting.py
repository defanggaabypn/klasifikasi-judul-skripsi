...
# OPTIMASI: Load model hanya ketika diperlukan
tokenizer = None
model = None
embedding_cache = {}
trained_knn = None
...
class KNNDetailedModel:
    ...
    def fit(self, X_train, y_train):
        ...
        # Tidak menyimpan X_train dan y_train untuk menghemat memori
...
# Fungsi untuk menghasilkan gambar confusion matrix dengan memori rendah
def generate_confusion_matrix_image(cm, labels, title, cmap, dpi=80):
    plt.figure(figsize=(6, 5), dpi=dpi)  # Kurangi DPI untuk menghemat memori
...
# Fungsi untuk menghasilkan gambar perbandingan metrik performance dengan memori rendah
def generate_performance_comparison(knn_metrics, dt_metrics, category_labels, dpi=80):
    # Create a figure for metrics comparison
    plt.figure(figsize=(10, 6), dpi=dpi)  # Kurangi DPI untuk menghemat memori
...
def process_file():
    ...
    if file and file.filename.endswith('.xlsx'):
        ...
        try:
            ...
            # OPTIMASI: Proses batch untuk embedding
            batch_size = 20  # Proses 20 judul sekali
            current_batch = []
            ...
            # Generate gambar confusion matrix dengan DPI rendah untuk menghemat memori
            knn_cm_img = generate_confusion_matrix_image(knn_cm, unique_labels, "Confusion Matrix - KNN", 'Blues', dpi=60)
            dt_cm_img = generate_confusion_matrix_image(dt_cm, unique_labels, "Confusion Matrix - Decision Tree", 'Greens', dpi=60)
            ...
            plt.figure(figsize=(8, 5), dpi=60)  # DPI rendah untuk menghemat memori
            ...
            try:
                ...
                if conn:
                    with conn.cursor() as cursor:
                        ...
                        # Simpan data judul dan hasil klasifikasi dalam batch
                        # untuk menghindari terlalu banyak query
                        batch_size = 50
                        title_batch = []
                        
                        for i, (title, label) in enumerate(zip(titles, labels)):
                            title_batch.append((title, label))
                            
                            if len(title_batch) >= batch_size or i == len(titles) - 1:
                                # Proses batch
                                for t, l in title_batch:
                                    # Dapatkan ID kategori
                                    cursor.execute("SELECT id FROM categories WHERE name = %s", (l,))
                                    category_result = cursor.fetchone()
                                    category_id = category_result['id'] if category_result else None
                                    
                                    if category_id:
                                        # Cek apakah judul sudah ada
                                        cursor.execute("SELECT id FROM thesis_titles WHERE title = %s", (t,))
                        ...
                        # Simpan hasil prediksi test set dalam batch
                        pred_batch = []
                        for i, result in enumerate(results_table):
                            pred_batch.append(result)
                            
                            if len(pred_batch) >= batch_size or i == len(results_table) - 1:
                                for res in pred_batch:
